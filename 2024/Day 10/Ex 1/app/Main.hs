...
countTrailheads topography =  let height = length topography
                              ...
                              in foldr ((+) . Set.size . findTrailheads topography width height) 0 [(x,y) | x <- [0..(width-1)] ,y <- [0..(height-1)], topography !! y !! x == 0]

findTrailheads::[[Int]]->Int->Int->(Int,Int)->Set.Set Pos
findTrailheads topography width height (x,y) = let topo = topography !! y !! x
                                        in if topo == 9 then Set.singleton (x,y)
                                         else foldr Set.union Set.empty [findTrailheads' (x', y') | (i,j) <- directions, let x' = x + i, let y' = y + j, isInBound x' y', topography !! y' !! x' == topo + 1]
  where
